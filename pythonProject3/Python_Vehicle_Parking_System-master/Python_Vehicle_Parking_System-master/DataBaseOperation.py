...
class DBOperation():
    ...
    def __init__(self):
        ...
        file.close()
        self.mydb = mysql.connector.connect(host="localhost", user=datadic['username'], passwd=datadic['password'],
    ...
    def CreateTables(self):
        ...
        cursor.execute("CREATE TABLE admin (id int(255) AUTO_INCREMENT PRIMARY KEY,"
                       "username varchar(30),password varchar(30),created_at varchar(30))")
        cursor.execute("CREATE TABLE slots (id int(255) AUTO_INCREMENT PRIMARY KEY,"
                       "vehicle_id varchar(30),space_for int(25),is_empty int(25))")
        cursor.execute("CREATE TABLE vehicles (id int(255) AUTO_INCREMENT PRIMARY KEY,"
                       "name varchar(30),mobile varchar(30),entry_time varchar(30),exit_time varchar(30),"
                       "is_exit varchar(30),vehicle_no varchar(30),vehicle_type varchar(30),"
                       "created_at varchar(30),updated_at varchar(30))")
        cursor.close()
    ...
    def AddVehicles(self, name, vehicleno, mobile, vehicle_type):
        ...
        if spacid:
            ...
            data = (
                name, mobile, str(currentdata), '', '0', vehicleno, str(currentdata), str(currentdata), vehicle_type)
            cursor = self.mydb.cursor()
            cursor.execute(
                "INSERT into vehicles (name,mobile,entry_time,exit_time,is_exit,vehicle_no,created_at,"
                "updated_at,vehicle_type) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                data)
            self.mydb.commit()
    ...
    def registerNewUser(self, username, password):
        ...
        val = (username, password, current_time)
        cursor.execute("INSERT INTO admin (username, password, created_at) VALUES (%s, %s, %s)", val)
        self.mydb.commit()
        cursor.close()
        return True
